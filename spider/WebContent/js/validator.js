...
function isMobilePhone(str) {
	if (str.match(/^(?:13\d|15\d|18\d|14\d|17\d)-?\d{5}(\d{3}|\*{3})$/) == null) {
		return false;
	} else {
		return true;
	}
...
function checkData(isRequire, dataId, tipName, min, max) {
	...
	if (isRequire && isNull(trim(data))) {
		ldDialog.alert(tipName + "\u4e0d\u80fd\u4e3a\u7a7a");
		return false;
	}
	if (!isNull(trim(data))) {
		if (!reg.test(trim(data))) {
			ldDialog.alert(tipName + "\u5fc5\u987b\u7531" + min + "\u81f3" + max + "\u4f4d\u5b57\u6bcd\u3001\u6570\u5b57\u6216\u6c49\u5b57\u7ec4\u6210!");
			return false;
		}
	}
	return true;
